...
export class OrdersService {
    private ORDERS_URL: string = `api/orders`;

    constructor(private httpClient: HttpClient) {
    }

    getAll() : Observable<Array<OrderModel>> {
        return this.httpClient.get<Array<OrderModel>>(`${this.ORDERS_URL}`)
            .map(response => OrderModel.fromServerResponseArray(response));
    }
