...
namespace nabe.order.management.DAL
{
    internal static class NabeDbContextExtensions
    {
        public static void EnsureSeedData(this NabeDbContext ctx)
        {
            ...
            {
                if (!ctx.Companies.Any() && !ctx.Customers.Any())
                {
                    ...
                    ctx.Customers.Add(new Customer { Name = "Baldík", Company = eyeLevel });;
                    ctx.Customers.Add(new Customer { Name = "Bešťák", Company = eyeLevel });;
                    ctx.Customers.Add(new Customer { Name = "Bureš", Company = eyeLevel });;
